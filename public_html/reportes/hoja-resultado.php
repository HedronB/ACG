...
if (isset($_GET['from']) && $_GET['from'] === 'cambios') {
    ...
    $linkVolver = match((int)$_SESSION['rol']) {
    1 => '/admin/menu_admin.php',
    default => '/user/menu_user.php'
};
}
...
if ($maquinaId > 0) {
    if ($rol === 1) {
        $stmt = $conn->prepare("SELECT ma_marca, ma_modelo FROM maquinas WHERE ma_id = :id AND ma_activo = 1");
        $stmt->execute([':id' => $maquinaId]);
    } else {
        $stmt = $conn->prepare("SELECT ma_marca, ma_modelo FROM maquinas WHERE ma_id = :id AND ma_empresa = :empresa AND ma_activo = 1");
        $stmt->execute([':id' => $maquinaId, ':empresa' => $empresaId]);
    }
    $maquinaInfo = $stmt->fetch(PDO::FETCH_ASSOC);
    if (!$maquinaInfo) { header('Location: /forms/form-hojaResultado.php'); exit(); }
} else { header('Location: /forms/form-hojaResultado.php'); exit(); }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        // Hora actual en UTC para guardar en BD
...
<head>
  ...
  <style>
    ...
    /* ── Overrides de reglas globales para esta página ── */
    /* El CSS global tiene "table td { display:flex }" que destruye las tablas de resultado */
    ...
    .excel-table td::before, .footer-table td::before { content: none !important; }

    /* Estilos del formulario visible */
...
<body>
  ...
  <div class="form-container">
    ...
    <form id="formPieza" enctype="multipart/form-data">
      ...
      <div class="form-actions">
        ...
        <button type="button" class="btn btn-pdf" onclick="imprimirReporteActual()">📥 Imprimir Hoja</button>
      </div>
    </form>
  </div>
  ...
  <script>
    ...
    window.onload = function() {
        // La función autoejecutable de arriba ya corre, pero si hay datos, cargamos inputs
        if (datosBD) {
    ...
    document.getElementById("formPieza").addEventListener("submit", function(e) {
      ...
      fetch('/reportes/hoja-resultado.php?id=<?= $maquinaId ?>', {
          method: 'POST',
